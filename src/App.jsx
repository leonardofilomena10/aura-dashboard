      ...
      <header className="sticky top-0 z-40 w-full border-b border-slate-800/80 bg-slate-950/85 backdrop-blur-md">
        ...
        <div className="max-w-7xl mx-auto px-6 py-4 flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            {/* Antigravity AI Conductor Toggle Button */}
            <button
              onClick={() => setShowAntigravityPanel(true)}
              className={`flex items-center gap-2 px-3.5 py-1.5 bg-gradient-to-r ${
                antigravityConnected 
                  ? 'from-indigo-650 via-emerald-600 to-indigo-550 hover:from-indigo-550 hover:to-emerald-500' 
                  : 'from-indigo-650 via-violet-650 to-indigo-550'
              } hover:scale-105 active:scale-95 transition-all shadow-md hover:shadow-indigo-500/25 border border-indigo-400/30 rounded-lg text-xs font-bold text-white relative group`}
              style={{ boxShadow: '0 0 12px rgba(99, 102, 241, 0.4)' }}
            >
              <Cpu className="w-3.5 h-3.5 text-indigo-200 animate-spin-slow" />
              <span>🤖 Antigravity AI</span>
              <span className={`w-2.5 h-2.5 rounded-full border border-slate-950 absolute -top-1 -right-1 ${
                antigravityConnected ? 'bg-emerald-400 animate-pulse' : 'bg-slate-550'
              }`} />
            </button>
                ...
                <div className="space-y-5 py-6">
                  ...
                  <div className="space-y-4 max-w-sm mx-auto">
                    <button
                      type="button"
                      onClick={() => {
                        setAntigravityUsername("admin.aura@gmail.com");
                        setAntigravityConnected(true);
                        setAntigravityMessages(prev => [
                          ...prev,
                          { sender: 'bot', text: `Protocole initialisé avec succès via Google SSO ! Bonjour admin.aura@gmail.com. Je contrôle désormais les paramètres d'Aura et les serveurs MCP. Dites par exemple : 'affiche le catalogue', 'active le thème émeraude' ou 'déploie le scénario'.` }
                        ]);
                        triggerToast("✓ Connecté avec succès via Google !");
                      }}
                      className="w-full py-2.5 bg-slate-900 hover:bg-slate-850 border border-slate-800 hover:border-slate-700 text-white text-xs font-bold rounded-xl transition-all shadow-lg flex items-center justify-center gap-2.5"
                    >
                    ...
                    <div className="flex items-center gap-3 py-1">
                      ...
                      <span className="text-[9px] text-slate-550 font-bold uppercase tracking-widest">ou</span>
                      <div className="h-[1px] bg-slate-800/80 flex-1"></div>
                    </div>
                  </div>
